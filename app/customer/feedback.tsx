...
export default function GiveFeedbackScreen() {
  ...
  const handleSubmit = async () => {
    if (!selectedTechnician) {
      Alert.alert('Please select a technician');
      return;
    }
    ...
    if (rating === 0 && trimmedComment === '' && trimmedComplaint === '') {
      Alert.alert('No feedback or complaint provided.');
      return;
    }
    ...
    try {
      ...
      if (!feedbackQuery.empty) {
        Alert.alert('You have already submitted a response for this technician.');
        return;
      }
      ...
      await addDoc(collection(db, 'feedbacks'), {
        ...
        complaint: trimmedComplaint || null,
        timestamp: serverTimestamp()
      });
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.label}>Complaint (if any):</Text>
      <TextInput
        style={[styles.input, styles.textArea]}
        placeholder="Mention your complaint if you have one..."
        multiline
        value={complaint}

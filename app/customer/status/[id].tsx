...
export default function CustomerStatus() {
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        setStatusData(customerData);

        // 🔽 Fetch technician name from 'technicians' collection using technicianId
        if (customerData.technicianId) {
          const techRef = doc(db, 'technicians', customerData.technicianId);
          const techSnap = await getDoc(techRef);
          if (techSnap.exists()) {
            const techData = techSnap.data();
            setTechnicianName(techData.name || 'Unnamed');
          }
        }
      }
    } catch (error) {
      console.error('Error fetching customer status or technician:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const { statusRoadmap, billUrl } = statusData;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.techName}>Technician: {technicianName}</Text>
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 22, fontWeight: 'bold', marginBottom: 20,    marginTop:40
},
  card: {
    width: '100%',
    backgroundColor: '#fff',

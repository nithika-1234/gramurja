...
export default function GenerateBill() {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      // }
      if (editingIndex !== null) {
  // No alert now — just update the item locally
  const updatedItems = [...billItems];
  updatedItems[editingIndex] = { ...updatedItems[editingIndex], ...newItem };
  setBillItems(updatedItems);
  setEditingIndex(null);
...
if (loading) {
  ...
  <Text style={styles.tableCellAmount}>Amount</Text>
        ...
        {billItems.map((item, index) => (
          ...
          // </View>
          <View key={index} style={styles.tableRowBill}>
  <Text style={styles.tableCellItem}>{item.item}</Text>
  <Text style={styles.tableCellQty}>{item.quantity}</Text>
  <Text style={styles.tableCellRate}>₹{item.price}</Text>
...
const styles = StyleSheet.create({
  ...
  tableCellQty: {
    width: '10%', // Smaller to shift left
    fontSize: 15,
    color: '#000',
    fontFamily: 'serif',
    borderRightWidth: 1,
